...
struct AchievementService {
    ...
    static func checkAchievements(for user: User, attempt: GameAttemptData, db: any Database) async throws -> [UserAchievement] {
        ...
        // 1. Проверка рекорда
        async let a = try await checkOrCreateHighScore(for: user, attempt: attempt, userAchievements: userAchievements, db: db)
        
        // 2. Проверка общего количества игр
        async let b = try await checkOrCreateTotalPlays(for: user, attempt: attempt, userAchievements: userAchievements, db: db)
        
        let results: [UserAchievement?] = [try await a, try await b]
    ...
    private static func checkOrCreateTotalPlays(
        ...
        if let totalPlaysAchievement = userAchievements.first(where: { $0.achievement.type == .totalPlays && $0.achievement.gameType == attempt.gameType }) {
            // Если есть — обновляем
            totalPlaysAchievement.progress += 1
            if totalPlaysAchievement.progress >= 10 && !totalPlaysAchievement.isUnlocked {
